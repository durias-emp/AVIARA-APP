...
export function useMaintenanceItems(aircraftId, hobbsCurrent, cyclesCurrent) {
  // Answers are stamped with the aircraft they describe, which is what makes
  // "loading" derivable: it is exactly the state of not yet holding an answer
  // for the aircraft being asked about. A separate loading flag set at the top
  // of the fetch had to be written synchronously inside an effect, and could
  // drift from the data besides.
  const [answer, setAnswer] = useState({ aircraftId: null, items: [], error: null })
  const fetchSeq = useRef(0)   // guards against out-of-order responses

  // Memoised because it feeds a useMemo below, and a fresh [] every render
  // would rebuild the scored list on every render along with it.
  const items = useMemo(
    () => (answer.aircraftId === aircraftId ? answer.items : []),
    [answer, aircraftId])
  const error = answer.aircraftId === aircraftId ? answer.error : null
  const loading = !!aircraftId && answer.aircraftId !== aircraftId

  const load = useCallback(async () => {
    // No write needed for "no aircraft": the derived values above already
    // answer [] for any id the stored answer was not fetched for.
    ...
    setAnswer({ aircraftId, items: err ? [] : (data ?? []), error: err ? err.message : null })
  }, [aircraftId])
