...
export default function PopupActions({ onSetDestination, onAddWaypoint, ident, name, lat, lon }) {
  // Both actions take the pilot somewhere else, so the popup that offered them
  // has done its job and closes with the press.
  //
  // Not cosmetic. A Leaflet popup keeps autoPan on, so an open one drags the
  // camera back to keep itself in view: the map would frame the new route and
  // the popup would immediately pan it away again, leaving a route drawn off
  // the edge of the screen and a card still sitting over the field. Closing it
  // is what lets the camera move at all. The drop-point popup also clears its
  // own marker, which this cannot do for it; this only takes down the panel.
  const map = useMap()
  ...
  return (
    <div style={{ marginTop: 9, display: 'flex', flexDirection: 'column', gap: 6 }}>
      {onSetDestination && (
        <button
          onClick={(e) => { e.stopPropagation(); map.closePopup(); onSetDestination(payload) }}
      ...
      {onAddWaypoint && (
        <button
          onClick={(e) => { e.stopPropagation(); map.closePopup(); onAddWaypoint(payload) }}
